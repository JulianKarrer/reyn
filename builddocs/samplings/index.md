...
- Similarly to Jian et al in "Blue Noise Sampling using an SPH–based Method"
...
- Project shifted particle to closest point on triangle it was sampled on (see "Real-Time Collision Detection" by Christer Ericson)
...
- In cases where mesh resolution greatly exceeds the fluid resolution, projecting to respective triangles limits freedom of movement across the mesh. The issue can be circumvented by downsampling input meshes, which is unproblematic since the spatial resolution of the simulation upwards bounds the level of detail anyways. Alternatively, create datastructure for "nearest point on any triangle of this mesh" queries to generalize the algorithm to completely arbitrary meshes.
...
Appears to be a very cheap and effective way to get blue noise samplings with minimal memory cost, as shown for the Stanford dragon model (20.1k samples) with many small triangles and non-convex features and test cube (38.4k samples) with large, flat areas in the following. 
