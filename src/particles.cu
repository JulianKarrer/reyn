...
void Particles::set_x(float3 *x)
{
    if (!gui)
    {
        throw std::runtime_error("Attempted to call set_x on Particles instance that is not managed by a GUI. Changing where the pointer to positions x points to does not make sense if the pointer is not managed by a mapping a OpenGL VBO but instead created by the Particles constructor.");
    }
    else
    {
...
Particles::Particles(GUI *_gui, float _h, float _rho_0)
    : h(_h), rho_0(_rho_0), gui(_gui), v(_gui->N), m(_gui->N), x(_gui) {};

Particles::Particles(const int N, float _h, float _rho_0)
    : h(_h), rho_0(_rho_0), v(N), m(N), x(N) {};
