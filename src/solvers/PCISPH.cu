...
__global__ void _write_pcisph_prs_acc(const float* __restrict__ xx,
    ...
    const float* __restrict__ m, const float* rho, const uint N, const K W,
    ...
    const BoundarySamples& bdy, const float dt)
{
    ...
    float k = (float)((δ / dt) / dt);
    std::cout << k << " " << δ << " " << dt << std::endl;
