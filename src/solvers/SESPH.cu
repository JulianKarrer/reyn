...
template <IsKernel K>
__global__ void _compute_accelerations_and_integrate(float3* x, float3* v,
    float* m, float* rho, uint N, const K W, const float k, const float rho_0,
    const float dt, const float nu, const float h)
{
    // compute index and ensure safety at bounds
    const auto i { blockIdx.x * blockDim.x + threadIdx.x };
    if (i >= N)
    ...
    for (uint j { 0 }; j < N; ++j) {
        ...
        a_i += (10.f * nu * m_j / rho_j * dot(v_ij, x_ij)
        ...
        // compute contribution to pressure acceleration by particle j at
        // particle i note the minus sign!
        a_i -= m_j * (p_i / (rho_i * rho_i) + p_j / (rho_j * rho_j)) * dW;
    }
...
void SESPH<K>::compute_accelerations(Particles& state, float dt)
{
    ...
    _compute_densities<K><<<BLOCKS(N), BLOCK_SIZE>>>(
        state.x.ptr(), state.m.ptr(), rho.ptr(), N, W);
