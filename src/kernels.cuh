...
/// @brief Base class of kernel functions for static polymorphism using CRTP design.
...
{
public:
    __host__ __device__ __forceinline__ C2(float h_bar) : Kernel(h_bar, 21.f / 2.f * M_1_PI) {};
...
{
public:
    __host__ __device__ __forceinline__ B3(float h_bar) : Kernel(h_bar, 16.f * M_1_PI) {};
    __host__ __device__ __forceinline__ static float w(float q)
    {
        const float t1{fmaxf(0.f, 1.f - q)};
...
{
public:
    __host__ __device__ __forceinline__ W6(float h_bar) : Kernel(h_bar, 1365.f / 64.f * M_1_PI) {};
    __host__ __device__ __forceinline__ static float w(float q)
    {
        const float t1{fmaxf(0.f, 1.f - q)};
        const float t1_2{t1 * t1};
...
{
public:
    __host__ __device__ __forceinline__ COS(float h_bar) : Kernel(h_bar, M_PI / (4.f * M_PI * M_PI - 30.f)) {};
    __host__ __device__ __forceinline__ static float w(float q)
    {
...
/// Macro used for explicit template instantiation, so that code templated on kernel function types is generated in all relevant translation units.
/// ADD NEW KERNELS TO THE MACRO :
