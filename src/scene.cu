...
Scene::Scene(const uint N_desired, const float3 min, const float3 max, const float3 bound_min, const float3 bound_max, const float rho_0, Particles &state)
{
...
__global__ void _hard_enforce_bounds(const float3 bound_min, const float3 bound_max, uint N, float3* x, float3* v)
{
    ...
    if (   bound_min.x <= x_i.x && x_i.x <= bound_max.x
        && bound_min.y <= x_i.y && x_i.y <= bound_max.y
        && bound_min.z <= x_i.z && x_i.z <= bound_max.z)
        return;
    // if execution reaches this point, find out what violation occured and fix it
